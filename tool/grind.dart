...
@Task('Get packages')
Future<void> pubGet({required String directory}) async {
  await _runProcess(
    'flutter',
    ['pub', 'get', directory],
  );
...
@Task('Update docs API')
Future<void> createDocsApi() async {
  final codeviewerPath =
      path.join(Directory.current.path, 'tool', 'codeparser_cli', 'main.dart');
  Dart.run(codeviewerPath);
}
